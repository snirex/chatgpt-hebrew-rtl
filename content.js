...
const dirBtn = makeButtonImage('direction-toggle-button','החלף כיוון כתיבת טקסט',imgPromptDir,'direction',toggleDirection);
const markdownBtn = makeButtonImage('markdown-toggle-button','החלף כיוון הצגת תשובות',imgMarkdownDir,'markdown',toggleMarkdownDirection);
...
function createButtons(){
  if(!btnGroup){
    ...
    btnGroup.style.cssText='position:absolute;display:flex;gap:10px;z-index:99999;';
  ...
  if(btnGroup.dataset.top != newTop || btnGroup.dataset.left != newLeft){
    ...
    [dirBtn, markdownBtn].forEach(btn=>{
	  ...
	  //setTimeout(()=>{btn.style.transform='translateY(0)';},500);
    });
  }
...
// עדכון מיקום כפתורים כשיש scroll או resize
window.addEventListener('scroll', createButtons);
window.addEventListener('resize', createButtons);

// עדכון כש-DOM משתנה (למשל כשטקסט או תיבת קלט מתווספים)
